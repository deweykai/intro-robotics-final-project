...
map_data = mapping_nowebots.map_data # 1 is for obstacles
###############################################################################
## Base Code
###############################################################################
class Node:
...
def state_is_valid(state):
    ...
    if map_data[int(state[1])][int(state[0])] == 1: return False # obstacle
...
def rrt(state_bounds, state_is_valid, starting_point, goal_point, K, delta_q):
    ...
    # rrt
    threshold = 1e-5
    node_list = [Node(starting_point, parent=None)] # Add Node at starting point with no parent
    for _ in range(K):
        target = np.array(get_random_valid_vertex(state_is_valid, state_bounds))
...
def plan_path(target_pos, bounds):
    end_p = mapping.coords_world_to_map((target_pos[0], target_pos[1]))
    start_p = mapping.coords_world_to_map((loc.pose_x, loc.pose_y))

    adj_map = convolve2d(mapping.map_data, np.ones((360, 360)), mode='same', boundary='fill', fillvalue=1)
    adj_map = (adj_map > (360**2 * 0.01)) * 1

    nodes = rrt(bounds, state_is_valid, start_p, end_p, K, np.linalg.norm(bounds/10.))
    path = smooth_path(nodes, adj_map)
